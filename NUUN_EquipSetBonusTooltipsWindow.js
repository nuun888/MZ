/*:-----------------------------------------------------------------------------------
 * NUUN_EquipSetBonusTooltipsWindow.js
 * 
...
Imported.NUUN_EquipSetBonusTooltipsWindow = true;
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            if (!!data) {
                ...
                for (const equip of data.SetBonusNumberEquipment || []) {
                    if (data.SetBonusEquipMaxNum > 1 && equip.SetNumberEquip <= setBonusSum) {
                        const rows = this.drawSetBonusNumberEquipment(equip, rect.x, y, rect.width);
                        y += lineHeight * rows;
                        contentsRows += rows;
                    }
                }
                if (data.SetBonusEquipMaxNum > 1 && data.SetBonusEquipMaxNum <= setBonusSum) {
