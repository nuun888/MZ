 ...
 * ありますので注意してください。desc1と記入した場合はアイテム、武器、防具のメモ欄に<desc1:ああああ>と記入したとき、記述欄タグ名にdesc1と
...
Game_System.prototype.initItemBookNumber = function() {
  ...
  if(!this._itembookData._itemBookNumber) {
    this._itembookData._itemBookNumber = [];
  }
  if(!this._itembookData._itemBookWeaponNumber) {
    this._itembookData._itemBookWeaponNumber = [];
  }
  if(!this._itembookData._itemBookArmorNumber) {
...
Game_System.prototype.isItemBook = function(item) {//データベース
  return item && item.name && !item.meta.NoBook;
};
...
Window_ItemBook_Index.prototype.unknownEnemyVisible = function(enemy) {
  return !UnknownVisible || (UnknownVisible && $gameSystem.isInItemBook(enemy));
};
