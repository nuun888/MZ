...
タイルセットごとにリージョンや地形タグを使用して複数のダメージ床を設定でき、HP・MP・TPへのダメージ、ステート付与、SE、フラッシュ、アニメーションなどを個別に設定できます。  


## 主な特徴
タイルセットごとに別々に設定が可能。  
マップ上のリージョンIDや地形タグに応じて、異なる床ダメージを設定できます。  
ダメージタイプを HP / MP / TP から選択可能。  
評価式によるダメージ設定で、固定値だけでなく、a.mhp * 0.02 のようにアクターの能力値を利用した計算式を設定が可能。  
NoDamage を指定すれば、ダメージなしでステート付与だけを行う床も作れます。  
床ダメージ時にステートを確率で付与が可能。  
ダメージを受けるアクターを指定が可能。  
床ごとにSE・フラッシュ・アニメーションを設定が可能。  
アニメーションはプレイヤーのみ、フォロワーを含めるなど再生対象も指定可能。  
特徴によって特定のダメージ床への耐性・弱点を設定が可能。  
