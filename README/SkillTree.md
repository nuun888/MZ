...
#### 前提スキル習得モード(Ver.1.7.0以降)
プラグインパラメータのスキルツリー設定の中のプラグインパラメータ、スキル設定の習得設定で前提スキルの習得モードを設定できます。  
`and`:全ての前提スキルが習得済み。未指定の場合は自動的にandが適用されます。  
`or`:いずれかの前提スキルが習得済み。  
`数値`:指定の数以上の前提スキルが習得済み。  
...
習得回数はリストIDと連動します。  
空白または、アイコンが設定されていない場合はNUUN_SkillTreeの設定が適用されます。一つでも設定されている場合は、NUUN_SkillTreeEXの設定が適用されます。  
例:リストID1が130 1回目までの習得済みアイコンがID130のアイコンで表示されます。  
リストID3が138 3回目の習得済みアイコンがID138のアイコンで表示されます。  

#### リセットスキルの設定
開いているスキルツリータイプのスキルをすべてリセットします。  
プラグインパラメータの「スキルリセットID」にスキルツリータイプをリセットするスキルを指定します。  
スキルのスキルタイプはなし推奨。  
スキルツリー設定のスキル設定でリセットスキルを選択してください。派生元スキルからリセットスキルに派生させることもできます。  

##### 消費アイテム
スキルツリータイプをリセットするための消費するアイテムを指定します。（1個消費固定）  
##### スキルツリー拡張Ver.1.4.0以降(有償版のみ)  
スキルツリー設定のスキル設定でリセットスキルでコストや条件の指定が有効になります。  
なお、SPの消費は無効になります。  
...
%1:獲得SP量  
%2:スキルポイント名  
戦闘終了時メッセージを空白にすることで、リザルト時のSPの報酬メッセージを無効化にできます。  
...
## スキルツリースキル項目表示カスタマイズ(有償版のみ)
スキルツリーウィンドウの習得スキル項目の表示をカスタマイズできます。  
また項目背景を画像に設定している場合は、画像の高さでの表示指定が出来ます。  
![画像](img/SkillTree6.png)    
![画像](img/SkillTree7.png)   

## スキルツリー拡張(有償版のみ)
#### コモンイベントの呼び出し(Ver.1.0.1以降)
スキルツリーウィンドウを閉じた後に指定のコモンイベントを実行します。  
即時コモンイベント発動を有効にした場合、スキル習得時にコモンイベントが実行されます。  
なお、メッセージやウェイトなどのコマンドは、即時コモンイベント発動を有効にした場合でもスキルツリーウィンドウが閉じてから実行されます。  

## スキルツリー自由配置(フルセット有償版のみ)
スキルツリーウィンドウに表示するスキルの配置を制約なく設定できるようにします。  
なお、NUUN_SkillTreeでのスキルツリー設定での設定は無効になります。  
![画像](img/SkillTree8.png)  

#### 設定
スキルツリー自由配置プラグインのプラグインパラメータのスキルツリー設でスキルツリーを設定します。NUUN_SkillTreeでのスキルツリー設定での設定は適用されませんのでご注意ください。  
NUUN_SkillTreeでの設定とは異なりスキルツリー設定で習得できるスキル全てを設定する必要があります。それは表示数位置を決めなければならないためです。  

##### 初期カーソルスキルID
対象のスキルツリータイプを選択し、スキルツリーウィンドウでのカーソルの初期位置のスキルIDを指定します。  
スキルが有効の場合はカーソルの初期位置がそのスキルのインデックスからになります。無効の場合は0の位置からになります。  

##### 表示X座標、表示Y座標
表示させる座標を指定します。X座標の場合は表示X列からの相対座標、Y座標の場合は表示Y列からの相対座標。  

##### 線のモード
線のモードを指定します。配列の番号は派生スキルの配列番号と同じです。未設定の場合は自動設定  
線のラインタイプは直線のみです。  
ベージュ曲線を適用するには、NUUN_SkillTreeで線のタイプをベージュ曲線(Type4)に設定してください。  
横→横  
![画像](img/hh.png)   
縦→縦  
![画像](img/vv.png)   
...
![画像](img/vh.png)   
##### カーソル設定
各カーソル操作で特定のスキルIDの位置に強制的にカーソル移動させることが出来ます。  
全てに0が設定されている、無効なスキルIDしか設定されていない場合は決定、キャンセル操作しか受け付けなくなりますので設定の際はご注意ください。  
0でカーソル移動無  
...
2026/4/18 Ver.1.0.4  
スキルタイプ名が適用されない問題を修正。  
2025/12/13 Ver.1.0.3  
線のモードがデフォルト設定のままだと線が表示されなくなる問題を修正。  
2025/12/6 Ver.1.0.2  
コスト変数の変数IDの設定を修正。  
2025/12/4 Ver.1.0.1  
スキル習得時に任意のSEを再生する機能を追加。  
スキル習得後にコモンイベントを呼び出す機能を追加。  
2025/11/28 Ver.1.0.0  
初版  
